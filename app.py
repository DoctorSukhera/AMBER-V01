...
def segmented_gauge_html(score=None, active=True):
    """Deterministic segmented AMBER semicircle for active demo or inactive preview state."""
    cx, cy, r = 200, 180, 142
    stroke_w = 23
    neutral = '#E8EDF3'
    palette = ['#A7D2F6','#83B9EB','#AFC1CB','#D6D8C8','#F6D572','#F4AD1D','#F0BF48','#E8EBF0','#E8EBF0','#E8EBF0']
    paths = []
    score_value = None if score is None else max(0.0, min(100.0, float(score)))
    for i in range(10):
        lo, hi = i*10, (i+1)*10
        start = 180 - lo*1.8 + 0.65
        end = 180 - hi*1.8 - 0.65
        color = palette[i] if active and score_value is not None and lo < score_value else neutral
        paths.append(f"<path d='{_arc_path(cx,cy,r,start,end)}' fill='none' stroke='{color}' stroke-width='{stroke_w}' stroke-linecap='butt'/>")
    if active and score_value is not None:
        center = f"<div class='locked-gauge-score'><div class='main'>{score_value:.1f}</div><div class='den'>/ 100</div></div>"
        caption = "<div class='locked-gauge-caption'>Illustrative AMBER software output</div>"
        aria = 'Illustrative AMBER software output gauge'
    else:
        center = "<div class='locked-gauge-score inactive'><div class='main'>—</div><div class='off'>Illustrative probability off</div></div>"
        caption = "<div class='locked-gauge-caption inactive'>Enable “Use illustrative demo probability”<br>to preview a demonstration score.</div>"
        aria = 'Illustrative AMBER probability preview is disabled'
    return f"""
    <div class='locked-gauge'>
      <div class='g50'>50</div><div class='g0'>0</div><div class='g100'>100</div>
      <svg viewBox='0 0 400 218' aria-label='{aria}'>
        <path d='{_arc_path(cx,cy,r,180,0)}' fill='none' stroke='{neutral}' stroke-width='{stroke_w}'/>
        {''.join(paths)}
      </svg>
      {center}
      {caption}
...
def build_amber_pdf(report):
    ...
    if report.get('illustrative_score') is not None:
        ...
        rows += [('Illustrative probability', 'not enabled')]
...
SVG_INFO = """<svg viewBox='0 0 64 64' fill='none' stroke='currentColor' stroke-width='3' stroke-linecap='round'><circle cx='32' cy='32' r='24'/><path d='M32 28v18'/><circle cx='32' cy='19' r='2.2' fill='currentColor' stroke='none'/></svg>"""
SVG_MOLECULAR_PAIR = """<svg viewBox='0 0 92 64' fill='none' stroke-linecap='round' stroke-linejoin='round'>
<g stroke='#347FD1' stroke-width='3.2'><path d='M10 40 22 28 34 34 45 20'/><circle cx='10' cy='40' r='5' fill='#D9EBFF'/><circle cx='22' cy='28' r='5' fill='#D9EBFF'/><circle cx='34' cy='34' r='5' fill='#D9EBFF'/><circle cx='45' cy='20' r='5' fill='#D9EBFF'/></g>
<g stroke='#E6A319' stroke-width='3.2'><path d='M49 43 60 31 72 38 84 24'/><circle cx='49' cy='43' r='5' fill='#FFF0C5'/><circle cx='60' cy='31' r='5' fill='#FFF0C5'/><circle cx='72' cy='38' r='5' fill='#FFF0C5'/><circle cx='84' cy='24' r='5' fill='#FFF0C5'/></g>
</svg>"""
SVG_OPTICAL = """<svg viewBox='0 0 72 72' fill='none' stroke-linecap='round' stroke-linejoin='round'>
<g stroke='#E8A315' stroke-width='2.8'><path d='M36 8v9M36 35v9M22 22h9M41 22h9M26 12l6 6M46 12l-6 6M26 32l6-6M46 32l-6-6'/><circle cx='36' cy='22' r='7' fill='#FFF0BE'/></g>
<path d='M16 50h40M20 50h32v8H20z' stroke='#1768B2' stroke-width='3.2'/><path d='M27 46h18' stroke='#1768B2' stroke-width='3.2'/>
</svg>"""
SVG_BRAIN_AMBER = """<svg viewBox='0 0 72 72' fill='none' stroke-linecap='round' stroke-linejoin='round'>
<g stroke='#347FD1' stroke-width='3'>
<path d='M34 13c-7-7-16-1-15 7-7 1-9 10-3 14-6 6-1 15 6 14 1 8 10 10 13 4V17c0-2-1-3-1-4Z'/>
<path d='M38 13c7-7 16-1 15 7 7 1 9 10 3 14 6 6 1 15-6 14-1 8-10 10-13 4V17c0-2 1-3 1-4Z'/>
<path d='M25 24c5 0 7 3 8 7M23 39c5-1 9 2 10 7M47 24c-5 0-7 3-8 7M49 39c-5-1-9 2-10 7'/>
</g>
<circle cx='44' cy='31' r='5.2' fill='#F4B42D'/><circle cx='44' cy='31' r='9' fill='none' stroke='#F4B42D' stroke-width='1.6' opacity='.45'/>
</svg>"""
...
if page == "Home":
    ...
    .locked-gauge .g0{{left:17px!important;bottom:41px!important;}}
    .locked-gauge .g100{{right:16px!important;bottom:41px!important;}}
    ...
    .st-key-amber_results_actions > div[data-testid="stVerticalBlock"]{{
        gap:16px!important;
    }}
    ...
    .calc-science-note-icon{{
        width:27px;height:27px;border-radius:50%;
        display:flex;align-items:center;justify-content:center;
        background:#0B59B0;color:#fff;
    }}
    .calc-science-note-icon svg{{width:16px;height:16px;}}
    .calc-science-note b{{color:#0B4EA1;}}
    ...
    with c3:
        with st.container(border=False, key="amber_scientific_panel"):
            ...
            with st.container(border=False, key="amber_science_bottom"):
                ...
                st.markdown(f"""<div class="calc-safeguard"><div class="calc-bulb">{SVG_BULB}</div><div><b>Scientific safeguard:</b> no final β coefficients, clinical thresholds, AUC, sensitivity, specificity, or model-lock date are shown until they are empirically derived and validated.</div></div><div class="calc-science-note"><div class="calc-science-note-icon">{SVG_INFO}</div><div><b>Feature interpretation:</b> R captures the relative Aβ42/Aβ40 composition, while M summarizes overall dual-biomarker abundance. These derived features are combined with age and sex in the AMBER model.</div></div>""", unsafe_allow_html=True)
    ...
    <style>
    /* ==========================================================
       PAGE 3 — DNA COMPASS / METHOD LOCKED VISUAL MATCH
       Scoped to Page 3 only. Shared hero/nav remain untouched.
       ========================================================== */
    .dna-title-row{
        display:grid;
    ...
    .dna-title-left{align-self:center;}
    .dna-kicker{
        color:#0B4EA1;
        font-size:12px;
        line-height:1;
    ...
    .dna-brand-message{text-align:right;align-self:center;padding-top:7px;}
    .dna-brand-main{color:#0B4EA1;font-size:13px;letter-spacing:5px;font-weight:500;white-space:nowrap;}
    .dna-brand-sub{color:#5D79A3;font-size:10.5px;letter-spacing:1.4px;margin-top:25px;white-space:nowrap;}
    .dna-brand-ai{position:relative;display:inline-block;padding:0 6px;}
    .dna-brand-ai:after{content:"";position:absolute;left:3px;right:3px;bottom:-13px;height:2px;background:#F4B42D;}
    ...
    .dna-purpose{
        ...
        border-radius:14px;
        background:linear-gradient(90deg,#F5FAFF 0%,#EDF6FF 100%);
        padding:0 28px;
        margin:0 0 24px 0;
    }
    .dna-purpose-icon{
        ...
        display:flex;align-items:center;justify-content:center;
        color:#fff;background:linear-gradient(180deg,#2C65AA,#0A438D);
        border:5px solid #D9EAFE;box-shadow:0 0 0 1px #8BB7E7;
    }
    .dna-purpose-icon svg{width:38px;height:38px;}
    .dna-purpose-sep{width:1px;height:72px;background:#93B9E1;}
    .dna-purpose-copy{padding-left:27px;}
    .dna-purpose-title{color:#06285F;font-size:16px;font-weight:850;margin-bottom:9px;}
    .dna-purpose-body{color:#315C91;font-size:15px;line-height:1.45;}
    ...
    .dna-step-num{
        ...
        background:#E6F2FF;color:#0B4EA1;
        display:flex;align-items:center;justify-content:center;
        font-size:15px;font-weight:850;
    }
    .dna-step-icon-disc{
        width:84px;height:84px;border-radius:50%;
        background:linear-gradient(180deg,#EEF7FF,#E7F2FD);
        display:flex;align-items:center;justify-content:center;
        margin:0 auto 5px;
        color:#1768B2;
    }
    .dna-step-icon-disc svg{width:59px;height:59px;display:block;}
    .dna-step-icon-disc.drop{color:#D95E55;}
    .dna-step-icon-disc.molecule svg{width:70px;height:52px;}
    .dna-step-icon-disc.optical svg{width:62px;height:62px;}
    .dna-step-icon-disc.amber-brain svg{width:61px;height:61px;}
    .dna-step-title{color:#06285F;font-size:15.5px;font-weight:850;line-height:1.1;margin-bottom:8px;}
    .dna-step-body{color:#526D8E;font-size:13.5px;line-height:1.36;max-width:185px;margin:0 auto;}
    .dna-arrow{display:flex;align-items:center;justify-content:center;color:#8DA7C4;}
    .dna-arrow svg{width:25px;height:25px;}
    ...
    .dna-lower-icon svg{width:54px;height:54px;}
    .dna-lower-title{color:#06285F;font-size:16px;font-weight:850;line-height:1.2;margin-bottom:8px;}
    .dna-lower-body{color:#526D8E;font-size:13.5px;line-height:1.4;}
    ...
    .dna-disclosure-icon svg{width:26px;height:26px;}
    .dna-disclosure strong{font-weight:850;}
    ...
    .footer.dna-footer>div:nth-child(1){text-align:left}
    .footer.dna-footer>div:nth-child(2){text-align:center}
    .footer.dna-footer>div:nth-child(3){text-align:right}

    @media(max-width:1100px){
        .dna-title-row{grid-template-columns:1fr;min-height:auto;}
        .dna-brand-message{display:none}
        .dna-workflow{
            grid-template-columns:1fr 20px 1fr 20px 1fr;
            row-gap:12px;
        }
        .dna-step-card{height:205px}
        .dna-lower-grid{gap:14px}
        .dna-disclosure{height:auto;min-height:48px;white-space:normal;line-height:1.35;padding-top:9px;padding-bottom:9px}
    }
    @media(max-width:760px){
        .dna-title{font-size:29px}
        .dna-sub{font-size:14px}
        .dna-purpose{height:auto;min-height:118px;grid-template-columns:76px 1px 1fr;padding:12px 14px}
        .dna-purpose-icon{width:58px;height:58px}
        .dna-purpose-sep{height:68px}
        .dna-purpose-copy{padding-left:16px}
        .dna-purpose-body{font-size:14px}
        .dna-workflow{display:block}
        .dna-step-card{height:auto;min-height:190px;margin-bottom:9px}
        .dna-arrow{height:20px;transform:rotate(90deg)}
        .dna-lower-grid{grid-template-columns:1fr}
        .dna-lower-card{height:auto;min-height:145px}
        .footer.dna-footer{height:auto;grid-template-columns:1fr;gap:4px}
        .footer.dna-footer>div{text-align:left!important}
    }
    </style>
    """, unsafe_allow_html=True)
    ...
    <div class="dna-title-row">
      <div class="dna-title-left">
        ...
        <div class="dna-sub">From blood-derived sample to a PET-informed estimate of cerebral amyloid positivity.</div>
      </div>
      <div class="dna-brand-message">
        <div class="dna-brand-main">BLOOD&nbsp;&nbsp;×&nbsp;&nbsp;<span class="dna-brand-ai">AI</span>&nbsp;&nbsp;×&nbsp;&nbsp;BRAIN HEALTH</div>
        <div class="dna-brand-sub">TRANSPARENT · REPRODUCIBLE · FOR A BRIGHTER TOMORROW</div>
      </div>
    </div>

    <div class="dna-purpose">
      <div class="dna-purpose-icon">{SVG_INFO}</div>
      <div class="dna-purpose-sep"></div>
      <div class="dna-purpose-copy">
        <div class="dna-purpose-title">Purpose of this page</div>
        <div class="dna-purpose-body">To show the molecular-to-computational architecture directly in the web interface, while keeping unpublished sequences, precise geometry, fabrication parameters and other patent-sensitive implementation details outside the public app.</div>
      </div>
    </div>

    <div class="dna-section-title">DNA Compass → AMBER</div>

    <div class="dna-workflow">
      <div class="dna-step-card">
        <div class="dna-step-num">01</div>
        <div class="dna-step-icon-disc drop">{SVG_DROP}</div>
        <div class="dna-step-title">Plasma</div>
        <div class="dna-step-body">Blood-derived<br>research sample.</div>
      </div>
      <div class="dna-arrow">{SVG_ARROW}</div>

      <div class="dna-step-card">
        <div class="dna-step-num">02</div>
        <div class="dna-step-icon-disc">{SVG_DNA}</div>
        <div class="dna-step-title">DNA Compass</div>
        <div class="dna-step-body">Programmable molecular<br>recognition architecture.</div>
      </div>
      <div class="dna-arrow">{SVG_ARROW}</div>

      <div class="dna-step-card">
        <div class="dna-step-num">03</div>
        <div class="dna-step-icon-disc molecule">{SVG_MOLECULAR_PAIR}</div>
        <div class="dna-step-title">Aβ40 + Aβ42</div>
        <div class="dna-step-body">Simultaneous selective<br>acquisition of the two<br>amyloid peptides.</div>
      </div>
      <div class="dna-arrow">{SVG_ARROW}</div>

      <div class="dna-step-card">
        <div class="dna-step-num">04</div>
        <div class="dna-step-icon-disc optical">{SVG_OPTICAL}</div>
        <div class="dna-step-title">Optical readout</div>
        <div class="dna-step-body">Quantitative reporter<br>signal converted into<br>concentrations.</div>
      </div>
      <div class="dna-arrow">{SVG_ARROW}</div>

      <div class="dna-step-card">
        <div class="dna-step-num">05</div>
        <div class="dna-step-icon-disc">{SVG_LAPTOP}</div>
        <div class="dna-step-title">Transform</div>
        <div class="dna-step-body">Ratio, R and M are<br>derived computationally.</div>
      </div>
      <div class="dna-arrow">{SVG_ARROW}</div>

      <div class="dna-step-card">
        <div class="dna-step-num">06</div>
        <div class="dna-step-icon-disc amber-brain">{SVG_BRAIN_AMBER}</div>
        <div class="dna-step-title">AMBER</div>
        <div class="dna-step-body">Future PET-trained model<br>estimates cerebral<br>amyloid positivity.</div>
      </div>
    </div>

    <div class="dna-lower-grid">
      <div class="dna-lower-card">
        <div class="dna-lower-icon">{SVG_BRAIN}</div>
        <div>
          <div class="dna-lower-title">Why Aβ40 and Aβ42?</div>
          <div class="dna-lower-body">This project focuses on the direct amyloid molecular pair (Aβ40 and Aβ42) because the reference endpoint is cerebral amyloid deposition. The key modelling question is whether retaining both measured dimensions adds reproducible value beyond the conventional ratio alone.</div>
        </div>
      </div>

      <div class="dna-lower-card">
        <div class="dna-lower-icon">{SVG_BAR_CHART}</div>
        <div>
          <div class="dna-lower-title">Contemporary comparator</div>
          <div class="dna-lower-body">p-tau217 can be included as a contemporary comparator where available. AMBER does not need to claim universal superiority of Aβ42/Aβ40 over p-tau217, rather, it examines the added value of the amyloid pair within a transparent and reproducible modelling framework.</div>
        </div>
      </div>
    </div>

    <div class="dna-disclosure">
...
@media(max-width:760px){
    ...
    ABOUT_GEAR = """<svg viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.9' stroke-linecap='round' stroke-linejoin='round'>
    ...
    .about5-title-left{align-self:center;}
    .about5-brand-message{text-align:right;align-self:center;padding-top:3px;}
    .about5-brand-main{color:#0B4EA1;font-size:13px;letter-spacing:5px;font-weight:500;white-space:nowrap;}
    .about5-brand-sub{color:#5D79A3;font-size:10.5px;letter-spacing:1.4px;margin-top:25px;white-space:nowrap;}
    .about5-brand-ai{position:relative;display:inline-block;padding:0 6px;}
    .about5-brand-ai:after{content:"";position:absolute;left:3px;right:3px;bottom:-13px;height:2px;background:#F4B42D;}
    .about5-kicker{
        color:#0B4EA1;
        font-size:12px;
        font-weight:850;
    ...
    @media(max-width:1150px){
        .about5-title-row{grid-template-columns:1fr;min-height:auto;}
        .about5-brand-message{display:none;}
    ...
    <div class="about5-title-row">
      <div class="about5-title-left">
        <div class="about5-kicker">SCIENTIFIC INFORMATION</div>
        <div class="about5-title">About AMBER</div>
      </div>
      <div class="about5-brand-message">
        <div class="about5-brand-main">BLOOD&nbsp;&nbsp;×&nbsp;&nbsp;<span class="about5-brand-ai">AI</span>&nbsp;&nbsp;×&nbsp;&nbsp;BRAIN HEALTH</div>
        <div class="about5-brand-sub">TRANSPARENT · REPRODUCIBLE · FOR A BRIGHTER TOMORROW</div>
      </div>
    </div>
