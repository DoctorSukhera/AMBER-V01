...
# ============================================================
# AMBER V0.1 — BEST FRONTEND PROTOTYPE
...
APP_STATUS = "BEST FRONTEND PROTOTYPE"
...
:root{
 ...
 --line:#DCE5F4;--softblue:#F1F6FF;--warn:#FFF8E7;--danger:#FFF0F0;--ok:#EFF9F1;
}
.block-container{max-width:1500px;padding-top:.72rem;padding-bottom:2rem}
header[data-testid="stHeader"]{background:rgba(255,255,255,.95);backdrop-filter:blur(8px)}
.top{background:linear-gradient(105deg,var(--navy),var(--navy2));color:white;border-radius:19px;padding:21px 27px;margin-bottom:12px;box-shadow:0 8px 26px rgba(7,29,73,.12)}
...
.hero h1{color:var(--navy);font-size:2.4rem;line-height:1.13;letter-spacing:-.035em;margin:.12rem 0 .8rem}
.hero p{color:#38465F;line-height:1.6;font-size:1rem;max-width:1000px}
...
.icon{width:40px;height:40px;border-radius:12px;background:#EEF4FF;display:flex;align-items:center;justify-content:center;color:#123F96;font-size:1.03rem;margin-bottom:.7rem;font-weight:850}
...
.success{border:1px solid #CAE6D1;background:var(--ok);color:#25613B;border-radius:13px;padding:13px 15px}
...
.qcpass{font-weight:800;color:#25613B}
.qcfail{font-weight:800;color:#9A3030}
.small{font-size:.79rem;color:#6F788B}
...
<div class="top">
  ...
  <div class="badges">
    ...
    <span class="badge">BEST FRONTEND PROTOTYPE</span>
    ...
    if len(edited) > 0:
        ...
        if len(tmp) > 0:
            ...
            with left:
                ...
                fig.update_layout(title="Biomarker concentrations", height=360, margin=dict(l=30,r=15,t=45,b=30))
                st.plotly_chart(fig, use_container_width=True)

            with right:
                fig2 = go.Figure()
                fig2.add_trace(go.Scatter(x=tmp["Visit"], y=tmp["Aβ42/Aβ40"], mode="lines+markers", name="Aβ42/Aβ40"))
                fig2.update_layout(title="Aβ42/Aβ40 ratio trajectory", height=360, margin=dict(l=30,r=15,t=45,b=30))
